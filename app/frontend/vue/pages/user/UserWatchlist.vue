<template>
  <div class="container">
    <ListHeader
        title="My Watchlist"
        :show-count="true"
        :items-count="watchlistTotalCount"
    ...
    <div v-else-if="!watchlist.length" class="mt-4 text-gray-700 dark:text-gray-300">
    ...
    <div v-if="watchlist.length > 0 || watchlistTotalCount > 0" class="mt-4 flex justify-center">
      <Pagination
...
const totalPages = computed(() => {
  return watchlistData.meta.value?.total_pages || 1
})

// TODO: the sorting logic should be refactored. Currently it's not getting used..
const { sortedMovies, handleFilterChange, handleOrderChange } = useSortedMovies(watchlist)
...
const handlePageChange = (page) => {
  ...
  interactionStore.fetchUserList('watchlist', page, username.value)
}

watch(() => authStore.isAuthenticated, (isAuthenticated) => {
  if (isAuthenticated && username.value) {
    interactionStore.fetchUserList('watchlist', currentPage.value, username.value)
  }
})
</script>