<template>
  <div class="relative w-[130px] md:w-[180px] overflow-hidden rounded-lg shadow-medium flex flex-col justify-between items-start">
    <div class="relative">
      <div class="absolute right-1.5 top-1.5 opacity-50 hover:filter-blue-invert transition-all">
        <svg class="w-[21px] h-[21px] md:w-[26px] md:h-[26px] cursor-pointer">
          <use xlink:href="#circle-more"></use>
        </svg>
      </div>

      <div class="min-h-[195px] md:min-h-[270px]">
        <router-link :to="{ name: 'movie-show', params: { slug: movie.slug }}">
          <img
              :src="coverImage"
              :alt="movie.title"
    ...
    <div class="h-[36px] md:h-[40px] mt-6 mb-1 px-2.5">
      <h2 class="font-SourceProBold line-clamp-2 text-sm md:text-base leading-5 mb-0.5">
        {{ movie.title }}
      </h2>
    </div>
    ...
    <span class="inline-block absolute bottom-[9px] right-[6px] text-gray-600 border border-gray-300 rounded text-[0.5rem] md:text-[0.7rem] px-1">
      {{ ageRating }}
    </span>
  </div>
</template>
...
/* Optional: Add a subtle hover effect for the more options button */
...
/* Optional: Add placeholder animation while image loads */
img {
