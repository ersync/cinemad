<template>
    ...
    >
        ...
        <button
            type="submit"
            class="py-3 px-5 bg-gradient-to-r from-[#0a192f] to-[#112240] text-white rounded-lg font-medium shadow-md hover:shadow-lg focus:ring-offset-2 focus:ring-[#233554] transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98]"
        >
            Apply Filters
        </button>
    </form>
...
.filter-section h2 {
    @apply text-sm font-medium text-slate-500 mb-2.5;
}

.filter-tag {
    @apply px-3 py-1.5 rounded-full border border-slate-200 bg-white text-sm transition-all duration-200 hover:bg-slate-100;
}

.form-select {
    @apply w-full rounded-md border-slate-300 py-2 pl-3 pr-10 text-sm focus:border-indigo-500 focus:ring-indigo-500;
}

.form-checkbox,
.form-radio {
    @apply rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 w-4 h-4;
}
</style>
